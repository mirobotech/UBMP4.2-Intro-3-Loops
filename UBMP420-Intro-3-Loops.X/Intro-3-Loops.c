...
unsigned char pwmLED3 = 127;    // LED3 brightness (PWM on-time) value
 ...
 * 
 * 1.   The main part of the program contains the 'outer' while(1) loop. Usually
 *      in C, brackets in a while or if structure are used contain a conditional
 ...
 *      different brightnesses can the LED have? Can you figure out the
 ...
 *      will be checked by the condition. Otherwise, the loop will never end 
 ...
 *      'for' loop. What is an advantage of using a for loop instead of a while
 *      loop? 
 *    
 ...
 *      Compile the program, download it into your UBMP4, and try it out. Does
 ...
 *      same name, but different values. (It's being done here to prove a 
 *      point!)
 * 
 ...
 *      the loop finishes. The if structure will let us know. Is LED D4 lit? If
 *      it is, what must the value of the global pwmPeriod variable be?
 * 
 ...
 *      contains no curly braces, and no code to run, but it does add a time
 ...
 *      increasing the time delay until the BEEPER pin toggles again in the next
 *      loop cycle.
 * 
 ...
 *      repeatedly cycle through its entire brightness range because the pwmLED3
 *      variable is allowed to either overflow, or underflow.
 * 
 *      Modify your program code so that pressing and holding SW2 will dim the
 *      LED until it is off, and keep if off, and pressing and holding SW3 will
 ...
 *      simultaneously. Add code to control LED D4 using SW4 and SW5 while
 *      continuing to use SW3 and SW2 to control LED D3.
 * 
 ...
 *      by lighting LED2 and LED5 at the start of your program and comparing
 *      their brightness to LED D3 and LED D4.
 * 
 ...
 *      PWM code to make a 'soft-start' program that slowly increases the PWM 
 *      on-time when a button is pressed. Can you also turn it off in a similar
 *      way?
 * 
 ...
 *      frequencies from low to high, or high to low when a button is pressed.
 */
